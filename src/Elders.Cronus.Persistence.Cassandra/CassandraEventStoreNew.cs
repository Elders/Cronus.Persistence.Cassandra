...
namespace Elders.Cronus.Persistence.Cassandra
{
    ...
    public partial class CassandraEventStore : IEventStore, IEventStorePlayer
    {
        private static readonly ILogger logger = CronusLogger.CreateLogger(typeof(CassandraEventStore));

        private const string LoadAggregateEventsQueryTemplate = @"SELECT rev,pos,data FROM {0} WHERE id = ?;";
        ...
        const int PublicEventsOffset = 5;
        ...
        public EventStream Load(IAggregateRootId aggregateId)
        {
            ...
            foreach (var row in result.GetRows())
            {
                ...
                {
                    var gg =  (IEvent)serializer.Deserialize(stream);
                    if (block.TryCompleteBlock(revision, position, gg, out AggregateCommit commit))
                    {
